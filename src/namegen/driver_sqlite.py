import sqlite3
import typing

connection: typing.Optional[sqlite3.Connection] = None

MAX_START: int
MAX_END: int


def _make_camel(w: str):
    return w[0].upper() + w[1:]


def start():
    global connection, MAX_START, MAX_END
    if connection is None:
        connection = sqlite3.connect('namegen.db')
        c = connection.cursor()
        c.execute('PRAGMA synchronous = OFF')
        c.execute('PRAGMA journal_mode = OFF')
        c.execute('SELECT (SELECT MAX(id) FROM start) + 1, (SELECT MAX(id) FROM end) + 1')
        MAX_START, MAX_END = c.fetchone()
        c.close()


def stop():
    global connection
    if connection is not None:
        connection.close()
        connection = None


def generate():
    new_start_id: int
    new_start: str
    new_end_id: int
    new_end: str

    if connection is None:
        raise ValueError('Connection Unavailable')

    with connection:
        cur = connection.cursor()
        cur.execute(
            """
            SELECT s.value, e.value, sq.next_value
            FROM state st
                JOIN sequences sq 
                    ON sq.start_id = st.next_start_id
                    AND sq.end_id = st.next_end_id
                JOIN start s ON s.id = sq.start_id
                JOIN end e ON e.id = sq.end_id
            """
        )
        start_part, end_part, sequence = cur.fetchone()
        cur.execute(
            """
            UPDATE sequences SET 
                next_value = ((21 * next_value + 1) % 10000),
                count = count + 1
            FROM state st
            WHERE start_id = st.next_start_id AND end_id = st.next_end_id
            """
        )
        cur.execute(
            f"""
            UPDATE state SET
                next_start_id = CASE WHEN next_start_id + 1 < {MAX_START} THEN next_start_id + 1 ELSE 1 END,
                next_end_id = CASE WHEN next_end_id + 1 < {MAX_END} THEN next_end_id + 1 ELSE 1 END,
                current_count = current_count + 1
            """
        )
        cur.close()
    return f'{_make_camel(start_part)}{_make_camel(end_part)}#{sequence:04d}'


__all__ = ['generate', 'start', 'stop']
